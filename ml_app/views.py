...
def get_accurate_model(sequence_length):
    ...
    if len(sequence_model) > 1:
        ...
        # Fallback: get the most accurate model regardless of sequence length
        list_models = [os.path.basename(m) for m in list_models]
        if list_models:
            accuracy = []
            for filename in list_models:
                acc = filename.split("_")[1]
                accuracy.append(acc)
            max_index = accuracy.index(max(accuracy))
            final_model = list_models[max_index]
        else:
    ...
    if request.method == 'GET':
        ...
        if video_upload_form.is_valid():
            ...
            if settings.DEBUG:
                ...
                with open(os.path.join(save_dir, saved_video_file), 'wb') as vFile:
                    shutil.copyfileobj(video_file, vFile)
                request.session['file_name'] = os.path.join(save_dir, saved_video_file)
            else:
...
def predict_page(request):
    if request.method == "GET":
        ...
        # Ensure the video is accessible via STATIC_URL or MEDIA_URL
        # If STATICFILES_DIRS includes 'uploaded_videos', then static files can find it.
        ...
        if not model_filename:
            return render(request, predict_template_name, {"error": "No AI model found in models directory.", "original_video": production_video_name})
            
        path_to_model = os.path.join(settings.PROJECT_DIR, 'models', model_filename)
        
        if not os.path.isfile(path_to_model):
            return render(request, predict_template_name, {"error": f"Model file not found: {model_filename}", "original_video": production_video_name})
        ...
        # No face detected
        if faces_found == 0:
            return render(request, predict_template_name, {"no_faces": True, "original_video": production_video_name})
        ...
        try:
            ...
            # Process all videos (though currently we only have one in path_to_videos)
            all_predictions = []
            for i in range(len(path_to_videos)):
                prediction = predict(model, video_dataset[i], './', video_file_name_only)
                all_predictions.append(prediction)
            
            # Use the first prediction result
            if all_predictions:
